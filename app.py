...
with mc[0]:
    ...
    st.session_state.map_ind = st.text_area(
        "适应症映射", value=st.session_state.map_ind, height=200, label_visibility="collapsed",
    ...
    st.session_state.map_hosp = st.text_area(
        "医院映射", value=st.session_state.map_hosp, height=200, label_visibility="collapsed")
with mc[1]:
    st.markdown("**科室归并映射**")
    st.session_state.map_dept = st.text_area(
        "科室映射", value=st.session_state.map_dept, height=200, label_visibility="collapsed",
        help="关键字=标准名，每行一条。")
    if st.button("重置科室映射为默认", key="reset_dept"):
        st.session_state.map_dept = C.DEFAULT_DEPT_MAP
        st.rerun()
    st.markdown("**医生清洗映射**（按 医院+医生 区分同名，如 张伟(主任) = 张伟）")
    st.session_state.map_doc = st.text_area(
        "医生映射", value=st.session_state.map_doc, height=200, label_visibility="collapsed")

# 清洗后规模预览（随上方映射实时更新）
_im, _dm, _hm, _cm = (C.parse_map(st.session_state.map_ind), C.parse_map(st.session_state.map_dept),
                      C.parse_map(st.session_state.map_hosp), C.parse_map(st.session_state.map_doc))
