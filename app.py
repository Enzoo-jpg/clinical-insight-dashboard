...
功能与纯前端版对齐：列名识别、时间窗口、本品/竞品、科室过滤、适应症多选、
四类清洗映射、医生四象限、机会市场、医院与品种/适应症分析、Excel 导出。
==========================================================================
"""
import io
import pandas as pd
...
def _init_state():
    ...
    ss.setdefault('map_dept', C.DEFAULT_DEPT_MAP)
    ss.setdefault('map_hosp', C.DEFAULT_HOSP_MAP)
    ss.setdefault('map_doc', C.DEFAULT_DOC_MAP)
...
with c2:
    ...
    if st.button("加载示例数据", width='stretch'):
        st.session_state.raw_df = C.mock_rows()
...
if raw_df is None:
    st.info("请上传销售明细，或点「加载示例数据」先体验。数据只在本次会话服务器内存中计算。")
    st.stop()

st.success(f"已读取 {len(raw_df):,} 行 · 列：{'、'.join(map(str, raw_df.columns))}")

# ========================================================= 2. 列映射
st.header("2. 列映射（底表列 → 标准字段）")
auto = C.auto_map_columns(list(raw_df.columns))
st.caption("红色为必填；系统已自动识别，可手动调整。真实底表建议：销售日期→销售时间、通用名→商品名称、销量数量→转化后数量。")
mapping = {}
mcols = st.columns(4)
opts = ['—'] + list(map(str, raw_df.columns))
for i, std in enumerate(C.STD):
    with mcols[i % 4]:
        default = auto.get(std, '')
        idx = opts.index(default) if default in opts else 0
        req = std in C.REQ
        label = f"{'🔴 ' if req else ''}{std}"
        sel = st.selectbox(label, opts, index=idx, key=f"map_{std}")
        if sel != '—':
            mapping[std] = sel

missing = [r for r in C.REQ if r not in mapping]
if missing:
    st.error("缺少必填字段映射：" + "、".join(missing))
    st.stop()

df_std = C.apply_mapping(raw_df, mapping)

# 首次载入自动导入四类映射中数据出现的全部值（仅补未覆盖）
if not st.session_state.get('_auto_imported'):
    ...
    st.session_state.map_dept = _import_all_values(df_std, '处方科室', st.session_state.map_dept)
    st.session_state.map_hosp = _import_all_values(df_std, '医疗单位', st.session_state.map_hosp)
    st.session_state.map_doc = _import_all_values(df_std, '处方医生', st.session_state.map_doc)
...
# 归一化后的候选值（供下拉/多选）
ind_map = C.parse_map(st.session_state.map_ind)
dept_map = C.parse_map(st.session_state.map_dept)
...
with st.expander("🧹 清洗映射（关键字=标准名，每行一条；已自动导入数据中全部值，可修改「标准名」列合并同类）", expanded=False):
    mc = st.columns(2)
    with mc[0]:
        st.markdown("**适应症归并映射**（可写大区，如 皮科/关节/消化）")
        st.session_state.map_ind = st.text_area("适应症映射", value=st.session_state.map_ind, height=180, label_visibility="collapsed")
        if st.button("重置适应症默认映射"):
            st.session_state.map_ind = C.DEFAULT_IND_MAP
            st.rerun()
        st.markdown("**科室归并映射**")
        st.session_state.map_dept = st.text_area("科室映射", value=st.session_state.map_dept, height=140, label_visibility="collapsed")
    with mc[1]:
        st.markdown("**医院清洗映射**（如 A院=A医院）")
        st.session_state.map_hosp = st.text_area("医院映射", value=st.session_state.map_hosp, height=180, label_visibility="collapsed")
        st.markdown("**医生清洗映射**（如 张伟(主任)=张伟；按医院+医生区分同名）")
        st.session_state.map_doc = st.text_area("医生映射", value=st.session_state.map_doc, height=140, label_visibility="collapsed")
...
cfg = {
    ...
    'deptMap': st.session_state.map_dept,
    'hospMap': st.session_state.map_hosp,
    'docMap': st.session_state.map_doc,
