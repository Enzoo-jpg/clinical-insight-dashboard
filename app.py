...
if all(c in df_std.columns for c in ('医疗单位', '处方科室', '处方医生')):
    ...
    if not _conflict.empty:
        ...
        st.caption(f"共 **{len(_conflict)}** 位医生：同一医院、同一医生名出现在 ≥2 个科室。"
                   "**直接在「标准科室」列填入要归并到的科室名**（留空 = 实为两个同名医生、不处理）。"
                   "全部填好后点下方按钮，一次性把规则自动追加到下方「组合映射」框，无需手动查找/复制。")
        _df_view = (_conflict.rename(columns={'医疗单位': '医院', '处方医生': '医生'})
        ...
        _edited = st.data_editor(
            ...
            column_config={
                ...
                '标准科室': st.column_config.TextColumn(
                    '标准科室（填=合并到这里，留空=不动）',
                    default='',
                    help='如确认是同一医生、应统一到一个科室：填写该科室名。「科室列表」里的所有变体都会被合并到此名。'
                ),
            },
        ...
        try:
            ...
            _n_marked = 0
        st.caption(f"已标注合并目标科室的行数：**{_n_marked}** / 共 {len(_edited)} 行")
        if st.button(f"🚀 将已标注的行追加为组合映射规则（{_n_marked} 条）",
                     key='commit_explore', disabled=(_n_marked == 0),
                     help='为每个标了标准科室的行，对其在「科室列表」里的每个变体生成一条 (医院,旧科室,医生) → (医院,标准科室,医生) 规则，追加到下方组合映射（自动去重）'):
